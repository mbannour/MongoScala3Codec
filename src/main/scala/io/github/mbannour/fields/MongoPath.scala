...
object MongoPath:
  ...
  /**
    ...
    *
    * The `.?` method lets you write `_.opt.?.field` to mean "walk into the option when present"
    * without contributing anything to the resulting path. It is defined `inline` and its body
    * is never evaluated.
    */
  object syntax:
  ...
  /**
    ...
    *
    * Example:
    *   MongoPath.of[Address](_.zipCode) // "zip" if annotated with @BsonProperty("zip")
    *
    * @tparam T the root case class
    * @param pick a lambda selecting a field chain, e.g. `_.a.b.c`
    * @return the resolved dot-separated path
    */
  ...
  private def ofImpl[T: Type](pick: Expr[T => Any])(using Quotes): Expr[String] =
    ...
    def collectSelects(term0: Term): List[(TypeRepr, Symbol)] =
      ...
      term match
        ...
        case Select(Inlined(Some(Apply(TypeApply(Ident(op), _), List(qual))), _, _), _) 
          if arglessTransparentOps.contains(op) =>
        ...
        case Select(qual, _) =>
          if arglessTransparentOps.contains(term.symbol.name) then
            collectSelects(qual)
          else
