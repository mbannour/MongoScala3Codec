...
There are two encoding paths, both of which correctly write the discriminator field:

**Path 1 — Encoding via the sealed trait codec** (e.g., `insertOne` on a `MongoCollection[Animal]`):
1. `SealedTraitCodecGenerator` writes `writeStartDocument()`
2. Writes discriminator field: `writer.writeString("_type", "Dog")`
3. Delegates to the concrete case class codec for the remaining fields
4. Writes `writeEndDocument()`

**Path 2 — Encoding via the concrete subtype codec directly** (e.g., `Updates.set("animal", Bird("Tweety", true))`):
1. The MongoDB driver calls `registry.get(classOf[Bird])` to obtain the `Bird` codec
2. The `Bird` codec (generated by `CaseClassCodecGenerator`) detects it belongs to a sealed hierarchy
3. It writes `writeStartDocument()` then the discriminator field: `writer.writeString("_type", "Bird")`
4. It writes the remaining fields and `writeEndDocument()`

Both paths produce an equivalent, self-describing BSON document that the sealed trait codec can decode.
...
## Partial Updates with Updates.set

When updating a field whose type is a sealed trait, use `Updates.set` with a concrete subtype value. The codec correctly includes the discriminator field so the stored document remains decodable:
...
val collection = database.getCollection[User]("users").withCodecRegistry(registry)

// Replace the pet field with a different subtype
// The encoded BSON will be: {"_type": "Bird", "name": "Tweety", "canFly": true}
collection.updateOne(
  Filters.eq("_id", userId),
  Updates.set("pet", Bird("Tweety", true))
).toFuture()

// Reading back works correctly because the discriminator is present
val user = collection.find(Filters.eq("_id", userId)).first().toFuture()
// user.pet will be Bird("Tweety", true) ✅
```

This works because `registerSealed[Animal]` also registers each concrete subtype codec with the `writeDiscriminator` flag enabled, ensuring discriminator-aware encoding no matter which codec path is used.
